...
class Glob:
    ...
    def match(self, path, basename=None, is_dir=None):
        """Match a path against the pattern. If the path matches the pattern, you must use the
           .include attribute to determine whether the path is to be added or removed from the
           result. path must be relative. basename and is_dir can be used to provide pre-computed
           values.
        """
        if basename is None:
    ...
    def get_group_key(self):
        """Return an object that can be used as a key for grouping different types of glob
           patterns.
        """
...
class GitIgnore:
    """Parse a .gitignore file according to the rules in the gitignore(5) manpage.
    """
    # XXX See gitignore(5). This class does not fully support all features of gitignore described
    ...
    def __init__(self, dirname, name):
        ...
        # paths and whether they are supposed to match directories only.
        self.patterns = []
        key = None
    ...
    def match(ignores, path, name, is_dir):
        """Match the path or basename against the sequence of patterns from all the ignore files
           and return True if the entry is to be excluded. Also return the path to the ignore file
           that contained the matching pattern.
        """
